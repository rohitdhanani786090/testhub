...
const appData = {
  ...
  "pricing": [
    ...
    },
    {
      ...
      "features": ["Custom Web Application", "E-commerce Functionality", "Dedicated Support", "API Integration"],
      "featured": false
    }
  ]
};
...
function populatePricing() {
    ...
    appData.pricing.forEach((plan, index) => {
        ...
        pricingCard.innerHTML = `
            ...
            <div class="pricing-price">
                <span class="dollar-sign">$</span>${plan.price}
            </div>
            <p class="pricing-description">${plan.description}</p>
            <ul class="pricing-features">
                ${plan.features.map(feature => `<li>${feature}</li>`).join('')}
            </ul>
            <button class="btn ${plan.featured ? 'btn--primary' : 'btn--outline'} btn--full-width">Choose Plan</button>
        `;
...
function populatePortfolio() {
    appData.portfolio.forEach((project, index) => {
        ...
        // Append first, then find the image container to initialize the effect
        portfolioGrid.appendChild(portfolioItem);

        // FIX: The hoverEffect library is causing a fatal error due to a dependency conflict with the included version of three.js.
        // This is commented out to allow the rest of the application to load.
        // To re-enable this, ensure compatible versions of three.js and hover-effect.js are used.
...
function populateTestimonials() {
    ...
    // Initialize carousel after populating
...
function initializeTestimonialCarousel() {
    ...
    function updateCarousel() {
        items.forEach((item, i) => {
            item.style.display = 'flex'; // Make sure all are visible for transform
            let offset = i - currentTestimonial;
            if (offset < -Math.floor(totalItems / 2)) offset += totalItems;
            if (offset > Math.floor(totalItems / 2)) offset -= totalItems;

            const scale = offset === 0 ? 1 : 0.8;
            const rotation = offset * 20; // degrees - Further reduced for a flatter, wider look
            const translateX = offset * 90; // percentage - Further increased for maximum spacing
            const zIndex = totalItems - Math.abs(offset);
            const opacity = Math.abs(offset) > 1 ? 0 : 1 - (Math.abs(offset) * 0.4);

            item.style.transform = `translateX(${translateX}%) rotateY(${rotation}deg) scale(${scale})`;
    ...
    const showNext = () => { currentTestimonial = (currentTestimonial + 1) % totalItems; updateCarousel(); };
    const showPrev = () => { currentTestimonial = (currentTestimonial - 1 + totalItems) % totalItems; updateCarousel(); };
    ...
    // A small delay ensures the initial transition is smooth after rendering
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (entry.target.classList.contains('stat-item') || entry.target.classList.contains('pricing-card')) {
    ...
    contactForm.addEventListener('submit', (e) => {
        ...
        .then(data => {
            ...
            setTimeout(() => {
                ...
                contactForm.reset();
                // Manually trigger blur on all controls to reset labels
                contactForm.querySelectorAll('.form-control').forEach(c => c.blur());
            }, 2000);
        })
        .catch(error => {
            // On error
            ...
            // Optionally re-enable the form after a delay
            setTimeout(() => { btnText.textContent = originalText; submitBtn.disabled = false; }, 3000);
        });
    });
...
function initializeNotificationPopup() {
    ...
    if (!popup || !closeBtn || !ctaBtn || sessionStorage.getItem('popupShown')) return;
...
// "Crazy and Attractive" Interactive Effects
function initializeInteractiveEffects() {
    ...
    if (hero && heroContent && heroShapes && window.innerWidth > 768) {
        hero.addEventListener('mousemove', (e) => {
            ...
            // Adjust these multipliers for more/less effect
            const contentRotateY = x * -5; // degrees
            const contentRotateX = y * 5;  // degrees
            const shapesTranslateX = x * 20; // pixels
            const shapesTranslateY = y * 20; // pixels

            heroContent.style.transform = `rotateY(${contentRotateY}deg) rotateX(${contentRotateX}deg)`;
            heroShapes.style.transform = `translateX(${shapesTranslateX}px) translateY(${shapesTranslateY}px)`;
        });

        hero.addEventListener('mouseleave', () => {
            heroContent.style.transform = `rotateY(0deg) rotateX(0deg)`;
            heroShapes.style.transform = `translateX(0px) translateY(0px)`;
        });
    }
    ...
    if (window.innerWidth > 768) {
        serviceCards.forEach(card => {
            card.addEventListener('mousemove', (e) => {
                ...
                const rotateX = (y / height - 0.5) * -20; // Max 10deg tilt
                const rotateY = (x / width - 0.5) * 20;   // Max 10deg tilt
